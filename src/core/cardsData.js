import Card from "./card";
import {
    KONSERWATYZM,
    PRAWICA,
    SOCJALIZM,
    LEWICA,
    LIBERALIZM,
    TUMIWISIZM
} from "./politicalViews";

const img = "https://ocdn.eu/pulscms-transforms/1/ieqktkqTURBXy9jNTJlZTU3MzExY2QwMGY2ZmVhNjc5ZTQ3M2RiMzdmMC5qcGVnkpUDAADNErHNCoSTBc0Djs0CXw";


export default [
    new Card(
        "001",
        "Wszyscy będziemy dobrze zarabiać!",
        {
            [KONSERWATYZM]: 5,
            [PRAWICA]: 3,
            [SOCJALIZM]: 2,
            [LEWICA]: 9,
            [LIBERALIZM]: 10,
            [TUMIWISIZM]: 5
        },
        img
    ),
    new Card(
        "002",
        "Zbudujemy trzy miliony mieszkań w ciągu roku!",
        {
            [KONSERWATYZM]: 0,
            [PRAWICA]: 4,
            [SOCJALIZM]: 8,
            [LEWICA]: 6,
            [LIBERALIZM]: 10,
            [TUMIWISIZM]: 5
        },
        img
    ),
    new Card(
        "003",
        "2000 złotych dla wszystkich obywateli, którzy nas poprą!",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 0,
            [SOCJALIZM]: 1,
            [LEWICA]: 6,
            [LIBERALIZM]: 2,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "004",
        "Czy się stoi, czy się leży, 200 zł się należy!",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 2,
            [SOCJALIZM]: 8,
            [LEWICA]: 10,
            [LIBERALIZM]: 5,
            [TUMIWISIZM]: 10
        },
        img
    ),
    new Card(
        "005",
        "Gruszki wyrosną na wierzbie. Obiecujemy!",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 10,
            [SOCJALIZM]: 1,
            [LEWICA]: 4,
            [LIBERALIZM]: 1,
            [TUMIWISIZM]: 8
        },
        img
    ),
    new Card(
        "006",
        "Damy mieszkania dla młodych małżeństw",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 1,
            [SOCJALIZM]: 9,
            [LEWICA]: 2,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 2
        },
        img
    ),
    new Card(
        "007",
        "Podatek liniowy",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 4,
            [SOCJALIZM]: 4,
            [LEWICA]: 3,
            [LIBERALIZM]: 0,
            [TUMIWISIZM]: 0
        },
        img
    ),
    new Card(
        "008",
        "Obniżymy VAT",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 4,
            [SOCJALIZM]: 10,
            [LEWICA]: 0,
            [LIBERALIZM]: 1,
            [TUMIWISIZM]: 6
        },
        img
    ),
    new Card(
        "009",
        "Zlikwidujemy bezrobocie - serio",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 4,
            [SOCJALIZM]: 4,
            [LEWICA]: 10,
            [LIBERALIZM]: 6,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "010",
        "Przywróćmy normalność, wygrajmy przyszłość",
        {
            [KONSERWATYZM]: 5,
            [PRAWICA]: 10,
            [SOCJALIZM]: 6,
            [LEWICA]: 1,
            [LIBERALIZM]: 1,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "011",
        "No pierwsze, to jak można prostytutkę zgwałcić?",
        {
            [KONSERWATYZM]: 9,
            [PRAWICA]: 8,
            [SOCJALIZM]: 7,
            [LEWICA]: 5,
            [LIBERALIZM]: 2,
            [TUMIWISIZM]: 2
        },
        img
    ),
    new Card(
        "012",
        "Dostęp do komputera dla każdego ucznia",
        {
            [KONSERWATYZM]: 2,
            [PRAWICA]: 3,
            [SOCJALIZM]: 5,
            [LEWICA]: 4,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "013",
        "Nikt Ci nie da tyle ile Ja Ci obiecam",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 10,
            [SOCJALIZM]: 10,
            [LEWICA]: 2,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 8
        },
        img
    ),
    new Card(
        "014",
        "Nie zrobimy koalicji z [wstaw tutaj kogo chcesz]",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 5,
            [SOCJALIZM]: 2,
            [LEWICA]: 5,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 3
        },
        img
    ),
    new Card(
        "015",
        "Becikowe",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 4,
            [SOCJALIZM]: 4,
            [LEWICA]: 1,
            [LIBERALIZM]: 10,
            [TUMIWISIZM]: 5
        },
        img
    ),
    new Card(
        "016",
        "Czyny, nie cuda",
        {
            [KONSERWATYZM]: 2,
            [PRAWICA]: 4,
            [SOCJALIZM]: 5,
            [LEWICA]: 7,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 10
        },
        img
    ),
    new Card(
        "017",
        "Druga Irlandia",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 3,
            [SOCJALIZM]: 9,
            [LEWICA]: 5,
            [LIBERALIZM]: 0,
            [TUMIWISIZM]: 8
        },
        img
    ),
    new Card(
        "018",
        "Zlikwidować senat!",
        {
            [KONSERWATYZM]: 3,
            [PRAWICA]: 1,
            [SOCJALIZM]: 10,
            [LEWICA]: 2,
            [LIBERALIZM]: 8,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "019",
        "Zmniejszenie biurokracji",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 7,
            [SOCJALIZM]: 9,
            [LEWICA]: 7,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "020",
        "Wolność, własność, sprawiedliwość",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 6,
            [SOCJALIZM]: 8,
            [LEWICA]: 5,
            [LIBERALIZM]: 8,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "021",
        "Nie przyjmiemy żadnych imigrantów",
        {
            [KONSERWATYZM]: 2,
            [PRAWICA]: 3,
            [SOCJALIZM]: 8,
            [LEWICA]: 1,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 5
        },
        img
    ),
    new Card(
        "022",
        "Milion nowych miejsc pracy",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 4,
            [SOCJALIZM]: 4,
            [LEWICA]: 0,
            [LIBERALIZM]: 2,
            [TUMIWISIZM]: 3
        },
        img
    ),
    new Card(
        "023",
        "Chłopak i dziewczyna - Normalna rodzina",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 6,
            [SOCJALIZM]: 2,
            [LEWICA]: 2,
            [LIBERALIZM]: 1,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "024",
        "500 zł na każde dziecko",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 2,
            [SOCJALIZM]: 4,
            [LEWICA]: 0,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 4
        },
        img
    ),
    new Card(
        "025",
        "Trzynasta pensja dla wszystkich!",
        {
            [KONSERWATYZM]: 3,
            [PRAWICA]: 10,
            [SOCJALIZM]: 10,
            [LEWICA]: 9,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 6
        },
        img
    ),
    new Card(
        "026",
        "Czyste powietrze",
        {
            [KONSERWATYZM]: 4,
            [PRAWICA]: 2,
            [SOCJALIZM]: 2,
            [LEWICA]: 7,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "027",
        "Apteki z \"awaryjną antykoncepcją\"",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 6,
            [SOCJALIZM]: 5,
            [LEWICA]: 9,
            [LIBERALIZM]: 0,
            [TUMIWISIZM]: 0
        },
        img
    ),
    new Card(
        "028",
        "50 lat i emeryturka",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 0,
            [SOCJALIZM]: 6,
            [LEWICA]: 2,
            [LIBERALIZM]: 2,
            [TUMIWISIZM]: 8
        },
        img
    ),
    new Card(
        "029",
        "Im mniej pieniędzy w budżecie państwa, tym lepiej, bo więcej zostaje w kieszeniach ludzi",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 7,
            [SOCJALIZM]: 2,
            [LEWICA]: 10,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "030",
        "Publiczna służba zdrowia",
        {
            [KONSERWATYZM]: 5,
            [PRAWICA]: 5,
            [SOCJALIZM]: 2,
            [LEWICA]: 7,
            [LIBERALIZM]: 10,
            [TUMIWISIZM]: 0
        },
        img
    ),
    new Card(
        "031",
        "Paliwo będzie tańsze",
        {
            [KONSERWATYZM]: 3,
            [PRAWICA]: 8,
            [SOCJALIZM]: 9,
            [LEWICA]: 6,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 3
        },
        img
    ),
    new Card(
        "032",
        "Bardzo się cieszę, że pochodzimy od różnej małpy",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 1,
            [SOCJALIZM]: 0,
            [LEWICA]: 7,
            [LIBERALIZM]: 0,
            [TUMIWISIZM]: 6
        },
        img
    ),
    new Card(
        "033",
        "Donald marzy żeby było miło",
        {
            [KONSERWATYZM]: 4,
            [PRAWICA]: 8,
            [SOCJALIZM]: 4,
            [LEWICA]: 7,
            [LIBERALIZM]: 10,
            [TUMIWISIZM]: 7
        },
        img
    ),
    new Card(
        "034",
        "Trzeba chronić polską rodzinę",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 7,
            [SOCJALIZM]: 10,
            [LEWICA]: 7,
            [LIBERALIZM]: 7,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "035",
        "100 zł na każdy tucznik",
        {
            [KONSERWATYZM]: 9,
            [PRAWICA]: 6,
            [SOCJALIZM]: 8,
            [LEWICA]: 6,
            [LIBERALIZM]: 1,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "036",
        "Zmień pracę, weź kredyt",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 7,
            [SOCJALIZM]: 0,
            [LEWICA]: 6,
            [LIBERALIZM]: 10,
            [TUMIWISIZM]: 4
        },
        img
    ),
    new Card(
        "037",
        "Jeśli minister finansów jest lubiany to znaczy, że jest zły",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 7,
            [SOCJALIZM]: 6,
            [LEWICA]: 0,
            [LIBERALIZM]: 10,
            [TUMIWISIZM]: 3
        },
        img
    ),
    new Card(
        "038",
        "Balcerowicz musi odejść",
        {
            [KONSERWATYZM]: 0,
            [PRAWICA]: 9,
            [SOCJALIZM]: 6,
            [LEWICA]: 8,
            [LIBERALIZM]: 7,
            [TUMIWISIZM]: 7
        },
        img
    ),
    new Card(
        "039",
        "Pensja minimalna na poziomie niemiec",
        {
            [KONSERWATYZM]: 0,
            [PRAWICA]: 2,
            [SOCJALIZM]: 9,
            [LEWICA]: 8,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "040",
        "Jak można zgwałcić prostytutkę?",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 1,
            [SOCJALIZM]: 7,
            [LEWICA]: 7,
            [LIBERALIZM]: 6,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "041",
        "Gospodarka oparta ne węglu",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 8,
            [SOCJALIZM]: 5,
            [LEWICA]: 8,
            [LIBERALIZM]: 6,
            [TUMIWISIZM]: 7
        },
        img
    ),
    new Card(
        "042",
        "Wolność wyznania, wolność sumienia",
        {
            [KONSERWATYZM]: 5,
            [PRAWICA]: 7,
            [SOCJALIZM]: 0,
            [LEWICA]: 5,
            [LIBERALIZM]: 8,
            [TUMIWISIZM]: 2
        },
        img
    ),
    new Card(
        "043",
        "KONSTYTUCJA!!!",
        {
            [KONSERWATYZM]: 4,
            [PRAWICA]: 7,
            [SOCJALIZM]: 6,
            [LEWICA]: 9,
            [LIBERALIZM]: 6,
            [TUMIWISIZM]: 4
        },
        img
    ),
    new Card(
        "044",
        "Są plusy dodanie i plusy ujemne",
        {
            [KONSERWATYZM]: 3,
            [PRAWICA]: 8,
            [SOCJALIZM]: 7,
            [LEWICA]: 5,
            [LIBERALIZM]: 8,
            [TUMIWISIZM]: 3
        },
        img
    ),
    new Card(
        "045",
        "Znam polskie drogi lepiej niż niejeden kierowca tira",
        {
            [KONSERWATYZM]: 4,
            [PRAWICA]: 8,
            [SOCJALIZM]: 0,
            [LEWICA]: 1,
            [LIBERALIZM]: 5,
            [TUMIWISIZM]: 7
        },
        img
    ),
    new Card(
        "046",
        "Wyginiecie jak dinozaury",
        {
            [KONSERWATYZM]: 2,
            [PRAWICA]: 8,
            [SOCJALIZM]: 0,
            [LEWICA]: 3,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "047",
        "Ja nie znam sprawy, nie męczcie mnie",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 5,
            [SOCJALIZM]: 1,
            [LEWICA]: 6,
            [LIBERALIZM]: 3,
            [TUMIWISIZM]: 8
        },
        img
    ),
    new Card(
        "048",
        "Gdybym był kobietą to byłbym lepszym premierem",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 3,
            [SOCJALIZM]: 3,
            [LEWICA]: 9,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 3
        },
        img
    ),
    new Card(
        "049",
        "Marna emerytura z ZUSa lepsza niż nic",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 10,
            [SOCJALIZM]: 2,
            [LEWICA]: 4,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 4
        },
        img
    ),
    new Card(
        "050",
        "500 zł na każdą krowę",
        {
            [KONSERWATYZM]: 2,
            [PRAWICA]: 10,
            [SOCJALIZM]: 10,
            [LEWICA]: 7,
            [LIBERALIZM]: 5,
            [TUMIWISIZM]: 6
        },
        img
    ),
    new Card(
        "051",
        "50 zł na każdą świnkę morską",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 2,
            [SOCJALIZM]: 6,
            [LEWICA]: 0,
            [LIBERALIZM]: 2,
            [TUMIWISIZM]: 10
        },
        img
    ),
    new Card(
        "052",
        "LGBT w każdej szkole",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 6,
            [SOCJALIZM]: 6,
            [LEWICA]: 10,
            [LIBERALIZM]: 5,
            [TUMIWISIZM]: 2
        },
        img
    ),
    new Card(
        "053",
        "Dopłaty do każdego nieużytku rolnego",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 0,
            [SOCJALIZM]: 8,
            [LEWICA]: 7,
            [LIBERALIZM]: 6,
            [TUMIWISIZM]: 10
        },
        img
    ),
    new Card(
        "054",
        "Ja bym chciał żeby pan Boska umył mi plecy",
        {
            [KONSERWATYZM]: 4,
            [PRAWICA]: 3,
            [SOCJALIZM]: 3,
            [LEWICA]: 1,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 10
        },
        img
    ),
    new Card(
        "055",
        "W 5 sekundzie słychać strzały",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 7,
            [SOCJALIZM]: 0,
            [LEWICA]: 6,
            [LIBERALIZM]: 6,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "056",
        "",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 5,
            [SOCJALIZM]: 5,
            [LEWICA]: 7,
            [LIBERALIZM]: 8,
            [TUMIWISIZM]: 0
        },
        img
    ),
    new Card(
        "057",
        "Darmowe żłobki dla dzieci matek niepracujących",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 4,
            [SOCJALIZM]: 8,
            [LEWICA]: 4,
            [LIBERALIZM]: 2,
            [TUMIWISIZM]: 3
        },
        img
    ),
    new Card(
        "058",
        "Ja jestem gorącym zwolennikiem prawicy, moje serce jest po prawej stronie",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 10,
            [SOCJALIZM]: 0,
            [LEWICA]: 6,
            [LIBERALIZM]: 1,
            [TUMIWISIZM]: 5
        },
        img
    ),
    new Card(
        "059",
        "Zwiększeni zasiłku dla bezrobotnych",
        {
            [KONSERWATYZM]: 2,
            [PRAWICA]: 9,
            [SOCJALIZM]: 10,
            [LEWICA]: 1,
            [LIBERALIZM]: 1,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "060",
        "Bóg, honor, dopłaty!",
        {
            [KONSERWATYZM]: 9,
            [PRAWICA]: 1,
            [SOCJALIZM]: 5,
            [LEWICA]: 3,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 6
        },
        img
    ),
    new Card(
        "061",
        "Wysoki Sejmie, nie po raz pierwszy staje mi...",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 3,
            [SOCJALIZM]: 8,
            [LEWICA]: 5,
            [LIBERALIZM]: 8,
            [TUMIWISIZM]: 5
        },
        img
    ),
    new Card(
        "062",
        "Pan nie dogonisz rynku? Przecież jest wolny!",
        {
            [KONSERWATYZM]: 3,
            [PRAWICA]: 10,
            [SOCJALIZM]: 2,
            [LEWICA]: 0,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "063",
        "Wolność, równość, SPRAWIEDLIWOŚĆ!",
        {
            [KONSERWATYZM]: 4,
            [PRAWICA]: 5,
            [SOCJALIZM]: 9,
            [LEWICA]: 8,
            [LIBERALIZM]: 2,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "064",
        "Nasz ryneczek Lidla zawsze będzie wolny...",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 10,
            [SOCJALIZM]: 6,
            [LEWICA]: 3,
            [LIBERALIZM]: 7,
            [TUMIWISIZM]: 3
        },
        img
    ),
    new Card(
        "065",
        "Może i mam horom córkę, ale sam ją wychowam!",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 5,
            [SOCJALIZM]: 5,
            [LEWICA]: 7,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 6
        },
        img
    ),
    new Card(
        "066",
        "Pójdziesz Pan ze mną nie iść na wybory?",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 5,
            [SOCJALIZM]: 6,
            [LEWICA]: 3,
            [LIBERALIZM]: 3,
            [TUMIWISIZM]: 10
        },
        img
    ),
    new Card(
        "067",
        "Tylko Donald może nas uratować...",
        {
            [KONSERWATYZM]: 2,
            [PRAWICA]: 8,
            [SOCJALIZM]: 7,
            [LEWICA]: 10,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 0
        },
        img
    ),
    new Card(
        "068",
        "Wszystko nasze zabrali, lewaki podłe",
        {
            [KONSERWATYZM]: 3,
            [PRAWICA]: 10,
            [SOCJALIZM]: 8,
            [LEWICA]: 2,
            [LIBERALIZM]: 3,
            [TUMIWISIZM]: 4
        },
        img
    ),
    new Card(
        "069",
        "A jak pójdziesz głosować, to na Bronka, czy tajemnica?",
        {
            [KONSERWATYZM]: 3,
            [PRAWICA]: 4,
            [SOCJALIZM]: 0,
            [LEWICA]: 1,
            [LIBERALIZM]: 8,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "070",
        "Januszek! Podaj tatusiowi karabin!",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 1,
            [SOCJALIZM]: 1,
            [LEWICA]: 1,
            [LIBERALIZM]: 7,
            [TUMIWISIZM]: 0
        },
        img
    ),
    new Card(
        "071",
        "Co mnie wojewodzie, to nie tobie... kasztelanie!",
        {
            [KONSERWATYZM]: 4,
            [PRAWICA]: 0,
            [SOCJALIZM]: 0,
            [LEWICA]: 0,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 8
        },
        img
    ),
    new Card(
        "072",
        "Jezus był socjalistą!",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 10,
            [SOCJALIZM]: 9,
            [LEWICA]: 0,
            [LIBERALIZM]: 1,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "073",
        "Nikogo nigdy o nic nie pytałem.",
        {
            [KONSERWATYZM]: 5,
            [PRAWICA]: 3,
            [SOCJALIZM]: 10,
            [LEWICA]: 10,
            [LIBERALIZM]: 3,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "074",
        "Na gębę to jest krem Nivea, ja proszę o dokumenty.",
        {
            [KONSERWATYZM]: 9,
            [PRAWICA]: 10,
            [SOCJALIZM]: 6,
            [LEWICA]: 10,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "075",
        "Starsi ludzie są przyzwyczajeni do traktowania wizyt u lekarza co dwa tygodnie jako rozrywki",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 0,
            [SOCJALIZM]: 1,
            [LEWICA]: 1,
            [LIBERALIZM]: 10,
            [TUMIWISIZM]: 10
        },
        img
    ),
    new Card(
        "076",
        "Klienci powinni kupować świadomie i rezygnować z najtańszych wędlin, które są najgorszej jakości",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 10,
            [SOCJALIZM]: 10,
            [LEWICA]: 5,
            [LIBERALIZM]: 7,
            [TUMIWISIZM]: 1
        },
        img
    ),
    new Card(
        "077",
        "Powinna być lewa noga i prawa noga. A ja będę pośrodku!",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 8,
            [SOCJALIZM]: 7,
            [LEWICA]: 4,
            [LIBERALIZM]: 0,
            [TUMIWISIZM]: 4
        },
        img
    ),
    new Card(
        "078",
        "Sorry, taki mamy klimat",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 1,
            [SOCJALIZM]: 4,
            [LEWICA]: 5,
            [LIBERALIZM]: 3,
            [TUMIWISIZM]: 7
        },
        img
    ),
    new Card(
        "079",
        "Jak to - oblałeś test przedsiębiorcy?",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 10,
            [SOCJALIZM]: 1,
            [LEWICA]: 3,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 0
        },
        img
    ),
    new Card(
        "080",
        "Immuniii... HIK! ...tet!",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 8,
            [SOCJALIZM]: 1,
            [LEWICA]: 4,
            [LIBERALIZM]: 8,
            [TUMIWISIZM]: 6
        },
        img
    ),
    new Card(
        "081",
        "Nie idziesz na wybory?!",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 9,
            [SOCJALIZM]: 10,
            [LEWICA]: 10,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 5
        },
        img
    ),
    new Card(
        "082",
        "Oczywiście, że lałem dzieci. Dzieci czasami trzeba karać",
        {
            [KONSERWATYZM]: 4,
            [PRAWICA]: 2,
            [SOCJALIZM]: 10,
            [LEWICA]: 9,
            [LIBERALIZM]: 5,
            [TUMIWISIZM]: 7
        },
        img
    ),
    new Card(
        "083",
        "Blokadę bym rozwalił, chłopom poobijał mordy, a ciągniki spalił!”",
        {
            [KONSERWATYZM]: 7,
            [PRAWICA]: 2,
            [SOCJALIZM]: 2,
            [LEWICA]: 4,
            [LIBERALIZM]: 5,
            [TUMIWISIZM]: 0
        },
        img
    ),
    new Card(
        "084",
        "Nikt normalny nie chodzi do telewizji...",
        {
            [KONSERWATYZM]: 10,
            [PRAWICA]: 0,
            [SOCJALIZM]: 3,
            [LEWICA]: 6,
            [LIBERALIZM]: 4,
            [TUMIWISIZM]: 10
        },
        img
    ),
    new Card(
        "085",
        "Weź już lepiej ten rutinoskorwin!",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 3,
            [SOCJALIZM]: 0,
            [LEWICA]: 9,
            [LIBERALIZM]: 6,
            [TUMIWISIZM]: 9
        },
        img
    ),
    new Card(
        "086",
        "Ty bereciasty kosmito!",
        {
            [KONSERWATYZM]: 2,
            [PRAWICA]: 2,
            [SOCJALIZM]: 9,
            [LEWICA]: 9,
            [LIBERALIZM]: 9,
            [TUMIWISIZM]: 4
        },
        img
    ),
    new Card(
        "087",
        "Woda ma to do siebie, że się zbiera i stanowi zagrożenie",
        {
            [KONSERWATYZM]: 1,
            [PRAWICA]: 3,
            [SOCJALIZM]: 7,
            [LEWICA]: 2,
            [LIBERALIZM]: 5,
            [TUMIWISIZM]: 6
        },
        img
    ),
    new Card(
        "088",
        "Posłanka? POSEŁKA masła!",
        {
            [KONSERWATYZM]: 6,
            [PRAWICA]: 6,
            [SOCJALIZM]: 10,
            [LEWICA]: 2,
            [LIBERALIZM]: 2,
            [TUMIWISIZM]: 8
        },
        img
    ),
    new Card(
        "089",
        "Kot jest mój, i będę z nim robił, co mi się podoba",
        {
            [KONSERWATYZM]: 5,
            [PRAWICA]: 0,
            [SOCJALIZM]: 0,
            [LEWICA]: 3,
            [LIBERALIZM]: 0,
            [TUMIWISIZM]: 2
        },
        img
    ),
    new Card(
        "090",
        "Lud jest głupi jak but z lewej stopy.",
        {
            [KONSERWATYZM]: 8,
            [PRAWICA]: 10,
            [SOCJALIZM]: 6,
            [LEWICA]: 1,
            [LIBERALIZM]: 5,
            [TUMIWISIZM]: 6
        },
        img
    )
];
